        ...
        if(controller.checkPassword(data)==1){
            if(textBox_newPW.getText().toString().equals("")){
                ...
                if(textBox_Email.getText().toString().indexOf(' ')>=0 || textBox_Email.getText().toString().indexOf('@')<0 || textBox_Email.getText().toString().indexOf('.')<0){//EMAIL CHECK 
                        controller.alertERROR("Wrong Email Format");
                    ...
                    if(controller.alertYesNo("<html>You will overwrite your information <br> Are you sure you want to update?</html>")==true){
                        
                        if(controller.setUserDataUpdate(sendData)==1){
                            controller.alertINFO("Done!");
                            displayUser(editUser.get(1).toString());
                            
                        }else{
                            controller.alertERROR("Error while Update - Try Again");
                        }
                ...
                if(textBox_newPW.getDocument().getLength()>=8 && textBox_newPW.getDocument().getLength()<=15 ){//PW CHECK
                    if(textBox_Email.getText().toString().indexOf(' ')>=0 || textBox_Email.getText().toString().indexOf('@')<0 || textBox_Email.getText().toString().indexOf('.')<0){//EMAIL CHECK
                        controller.alertERROR("<html>Password :<br> field need to have <br> at least 8 with max of 15 digits</html>");
            ...
            controller.alertERROR("Wrong Password inserted");
        }
...
/*Simple add +1 to booking number */
    ...
    displayUser(editUser.get(1).toString());
    controller.alertINFO("You now have "+Integer.toString(value)+" Booked Classes");
    ...
    displayUser(editUser.get(1).toString());
        ...
        if(yearDays<365){//less 1 year no revoke
            ...
            if(editUser.get(5).equals("silver")){
            controller.alertINFO("<html>A year pass <br>Your membership is <u>"+editUser.get(5)+"</u> so it's free, to upgrade to gold you need 10 active booking classes per year <br><br>Your Number of Booked Classes will now be reseted.</html>");
            controller.revokeUser(editUser.get(1).toString(),Integer.toString(actDate));
                
            }else if(editUser.get(5).equals("gold")){
            controller.alertWARR("<html>Your membership as <u>"+editUser.get(5)+"</u> ended because a year pass <br> Your Number of Booked Classes will now be reseted and silver membership will be added .</html>");
            controller.revokeUser(editUser.get(1).toString(),Integer.toString(actDate));
                
            }else if(editUser.get(5).equals("platinum")){
            controller.alertWARR("<html>Your membership as <u>"+editUser.get(5)+"</u> ended because a year pass <br> Your Number of Booked Classes will now be reseted and silver membership will be added <br> to keep Platinum membership a fee of £100 is required.</html>");
            controller.revokeUser(editUser.get(1).toString(),Integer.toString(actDate));
